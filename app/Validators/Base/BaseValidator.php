...
class BaseValidator extends LaravelValidator
{
    ...
    {
        return ['rules' => [$this->getModel()->getKeyName() => 'required' . $this->_getExistInDbRule($this->getModel()->getTableName())] + $this->_getRulesDefault(), 'messages' => $this->_getMessagesDefault()];
    }
    ...
    protected function _buildShowRules()
    {
        return [
            'rules' => $this->_buildRules(array(
                $this->getModel()->getKeyName() => 'required' . $this->_getExistInDbRule(),
            ), false),
            'messages' => [
                $this->getModel()->getKeyName().'.exists' => trans('validation.exists', ['attribute' => $this->getModel()->getModelName()]),
            ]
        ];
    }
    ...
    {
        if (!is_array($data)) {
            $data = array($this->getModel()->getKeyName() => $data);
        }
    ...
    {
        if(empty($fields)){
            $fields = [$this->getModel()->getKeyName()];
        }
    ...
    {
        $fs = ['email', $this->getData($this->getModel()->getKeyName()), $this->getModel()->getKeyName()];
        if (!empty($fields)) {
            $tmpField = array_first($fields);
